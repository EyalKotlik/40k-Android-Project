...
def main():
    ...
    for desc in data["description"]:
        ...
        data['description'][index] = re.sub(
            "<[^<>]*>", "", data['description'][index].lower())  # removes links
        # format varius things
        data['description'][index] = re.sub(
            "&lt;|&gt;", "-key-", data['description'][index])  # marks a keyword
        data['description'][index] = re.sub(
            "core unit", "-cu-", data['description'][index])  # core unit
        data['description'][index] = re.sub(
            "enemy", "-eu-", data['description'][index])  # enemy unit
        data['description'][index] = re.sub(
            "friendly", "-fu-", data['description'][index])  # friendly unit
        data['description'][index] = re.sub(
            "this model", "-ma-", data['description'][index].lower())  # model ability
        data['description'][index] = re.sub(
            "this unit", "-ua-", data['description'][index].lower())  # unit ability
        data['description'][index] = re.sub(
            "\+ invulnerable save", "-isv-", data['description'][index].lower())  # invulnerable save
        data['description'][index] = re.sub(
            "an unmodified hit roll of", "-uhr-", data['description'][index].lower())  # unmodified hit roll
        data['description'][index] = re.sub(
            "an unmodified wound roll of", "-uwr-", data['description'][index].lower())  # unmodified wound roll
        data['description'][index] = re.sub("subtract 1 from that attack’s hit roll",
                                            "-s1hr-", data['description'][index].lower())  # subtract 1 from hit roll
        data['description'][index] = re.sub("subtract 1 from that attack’s wound roll",
                                            "-s1wr-", data['description'][index].lower())  # subtract 1 from wound roll
        data['description'][index] = re.sub("each time an attack is made|each time an attack is allocated to a model",
                                            "-ea-", data['description'][index].lower())  # enemy attack [is made]
        data['description'][index] = re.sub("each time a melee attack is made against|against melee attacks",
                                            "-ema-", data['description'][index].lower())  # enemy melee attack [is made]
        data['description'][index] = re.sub("each time a ranged attack is made against|against ranged attacks",
                                            "-era-", data['description'][index].lower())  # enemy ranged attack [is made]
